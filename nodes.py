...
def _antithetic_noise(noise_sampler, sigma_from, sigma_to, count: int) -> List[Tensor]:
    ...
    Returns a list of exactly 2*count tensors (original + negated pairs).

    IMPORTANT: Callers must ensure the final number of noise paths used is even
    to maintain zero-mean property. If noise_paths is odd, antithetic sampling
    is disabled and a warning should be issued (handled at call sites).
    """
    noises = []
...
def _get_noise_direction(
    ...
    """Generate and merge noise paths into a single direction with optional variance reduction.

    Returns the merged noise direction scaled by boost.
    """
    if variance_reduction == "antithetic":
        # Antithetic sampling requires even number of paths for unbiased estimator.
        # If odd, fall back to standard sampling for the last path.
        pair_count = noise_paths // 2
        if pair_count > 0:
            noises = _antithetic_noise(noise_sampler, sigma_from, sigma_to, pair_count)
            # If noise_paths is odd, append one additional independent noise
            if noise_paths % 2 == 1:
                noises.append(noise_sampler(sigma_from, sigma_to))
        else:
            # Only 1 path requested - antithetic doesn't help, just use standard
    ...
    if parameterization == "edm":
        ...
        if enhanced_active:
            direction = _get_noise_direction(
                noise_sampler, sigma_a, sigma_b, noise_paths, merge_mode,
                noise_normalization, variance_reduction, boost
            )
...
def _apply_langevin_corrector(
    ...
    The update follows the standard unadjusted Langevin algorithm (ULA):
    x_{k+1} = x_k + (δ/2) * ∇log p(x_k) + sqrt(δ) * z

    where δ = 2 * corrector_eta * sigma is the effective step size,
    and the score is approximated as (denoised - x) / sigma^2.
    """
    if corrector == "none" or corrector_steps <= 0 or sigma <= 0.0:
        return x

    # Effective Langevin step size: δ = 2 * eta * sigma
    # This gives: (δ/2) * score = eta * sigma * (denoised - x) / sigma^2
    #                          = eta * (denoised - x) / sigma
    base_step_size = corrector_eta * sigma

    for _ in range(corrector_steps):
        denoised_current = model(x, sigma_tensor * s_in, **extra_args)
        score = (denoised_current - x) / (sigma ** 2 + _EPS)

        step_size = base_step_size
        if corrector == "langevin_dynamic":
            # Dynamic step size based on local gradient magnitude
            # Use a copy to avoid mutating the base step size across iterations
        ...
        # x += step_size * score + sqrt(2 * step_size) * noise
        # This is equivalent to ULA with δ = 2 * step_size
...
def sample_euler_a2(
    ...
    for i in trange(total, disable=disable):
        ...
        if (
            ...
            # Apply Langevin corrector after ancestral substeps
            x = _apply_langevin_corrector(
                model, x, sigma_ip1_f, sigma_ip1, s_in, extra_args,
                noise_sampler, corrector, corrector_steps, corrector_eta
            )
            continue
        ...
        if deterministic:
            x = x_det
            # Apply corrector even for deterministic path (refines the sample)
            x = _apply_langevin_corrector(
                model, x, sigma_ip1_f, sigma_ip1, s_in, extra_args,
                noise_sampler, corrector, corrector_steps, corrector_eta
            )
        ...
        if parameterization == "edm":
            ...
            if sigma_up <= 0.0:
                ...
                if enhanced_active:
                    direction = _get_noise_direction(
                        noise_sampler, sigma_i, sigma_ip1, noise_paths, merge_mode,
                        noise_normalization, variance_reduction, boost
                    )
        ...
        # Apply Langevin corrector after renoise for both parameterizations
        x = _apply_langevin_corrector(
            model, x, sigma_ip1_f, sigma_ip1, s_in, extra_args,
            noise_sampler, corrector, corrector_steps, corrector_eta
        )
            ...
            "optional": {
                ...
                "variance_reduction": (
                    list(VARIANCE_REDUCTION_MODES),
                    {
                        ...
                        "Best results with even noise_paths (2, 4, 6, 8).",
                    },
                ),
            },
        }

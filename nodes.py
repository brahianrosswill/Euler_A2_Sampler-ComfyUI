...
- Higher-order integration of the deterministic down-step (the probability-flow
  ...
  heun / dpm2 (2nd order), rk3 (3rd order), rk4 (4th order).
...
METHODS = ("euler", "midpoint", "ralston", "heun", "dpm2", "rk3", "rk4")
...
def _effective_substeps(substeps, progress, active_start, active_end, fade):
...
def sample_euler_a2(
    ...
    for i in trange(total, disable=disable):
        ...
        if not deterministic and substep_mode == "ancestral" and effective_substeps > 1:
            points = _subdivide_sigmas(sigma_i_f, sigma_ip1_f, effective_substeps, substep_spacing)
        ...
        down_substeps = effective_substeps if (substep_mode == "deterministic" or deterministic) else 1
...
class EulerA2Sampler:
            ...
            "optional": {
                ...
                "method": (list(METHODS), {"default": "euler",
                                           "tooltip": "Integration order of the deterministic down-step: "
                                                      ...
                                                      "rk3 (3rd order, 3 evals), rk4 (4th order, 4 evals) per substep."}),
                ...
                "substep_active_start": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.05, "round": False,
                                                   "tooltip": "Fraction of sampling where internal substeps begin. "
                                                              "e.g. 0.0 = from the first step, 0.3 = skip the first 30 %."}),
                "substep_active_end": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.05, "round": False,
                                                 "tooltip": "Fraction of sampling where internal substeps end. "
                                                            "e.g. 0.5 = stop after the first half of steps."}),
                "substep_fade": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.05, "round": False,
    ...
    DESCRIPTION = ("Euler ancestral sampler that merges several noise paths per step and extrapolates "
                   "along their shared direction, with higher-order integration methods, internal "
                   "substepping, and practical substep scheduling for smoother, more efficient "
                   "ancestral sampling.")
    ...
    def get_sampler(self, eta, s_noise, extrapolation, noise_paths=2, merge_mode="mean",
                    ...
                    substep_active_start=0.0, substep_active_end=1.0, substep_fade=0.0):
